...
- 검증 중 확인된 제약:
  ...
  - 현재 저장소는 이 VTS 제약을 polling adapter에서 empty `open_orders` snapshot으로 흡수하도록 보정했습니다.
  ...
  - `2026-04-20` 현재 저장소의 `execution/kis_api.py`에서 국내 현금주문/취소 POST 계약을 보강했고, 공식 GitHub 샘플 기준 `VTTC0012U`(매수), `VTTC0011U`(매도), `VTTC0013U`(정정취소) TR을 기본값으로 사용함
  - 같은 검증에서 `hashkey`는 best-effort로 부가하되 실패 시 주문 본호출은 계속 진행하도록 정리했고, legacy TR 계약이 필요한 경우 `tr_id` override를 허용함
  - `2026-04-20 09:4x KST` 삼성전자(`005930`) 1주 낮은 지정가 매수 후 즉시 취소를 현재 코드 경로로 재검증했고, 주문 `40600000 / 모의투자 매수주문이 완료 되었습니다.`, 취소 `40630000 / 모의투자 취소주문이 완료 되었습니다.` 응답과 로컬 `orders.status=cancelled`를 확인함
  - 즉시 취소 첫 시도에서는 `EGW00201 / 초당 거래건수를 초과하였습니다.`가 1회 발생했으며, 짧은 재시도로 정상 취소됨
...
- `retryable`
  ...
  - 처리: submit/cancel write path에서 짧은 자동 재시도 후에도 실패하면 `retry_count` 증가, `max_submit_retries` 미만이면 `validated` 유지, 초과 시 `failed`
- `terminal`
...
- polling 순서는 `get_valid_token -> KR broker fill sync -> account/open_orders/cash 조회 -> build_polling_snapshot -> reconcile_snapshot`으로 고정합니다.
- 현재 저장소 기준 broker fill auto-sync는 KR만 지원하며, `주식일별주문체결조회`의 cumulative fill을 `ExecutionFill` delta로 변환해 `fill_processor`에 반영합니다.
- 현재 저장소 기준 KR VTS `open_orders` 조회는 미지원 응답을 adapter에서 empty payload로 흡수합니다.
- 현재 저장소 기준 US 자동 fill ingestion은 미구현으로 유지합니다.
- mismatch 시 `order_manager.flag_reconciliation_hold()`를 호출하고 `trading_blocked=True`를 유지합니다.
- polling read-only broker query는 rate limit / temporary broker error에 대해 짧은 재시도 후에도 실패한 경우에만 polling failure로 집계합니다.
...
- 취소 대상은 `submitted`, `partially_filled` 상태인 주문으로 한정합니다.
- 국내(KR)는 `kis_order_no`와 `kis_order_orgno`가 모두 저장된 주문만 브로커 취소 대상으로 사용합니다.
- 미국(US)은 기존처럼 `kis_order_no` 기준 취소 계약을 유지합니다.
- 취소 요청 순서는 `cancel_order -> normalize_cancel_result -> request_cancel -> confirm_cancel`로 고정합니다.
- 취소의 broker write path는 retryable rate-limit/temporary 오류에 대해 짧은 자동 재시도를 수행합니다.
...
- `strategy.dual_momentum.DualMomentumStrategy`는 historical/backtest 호환용 구현으로 유지합니다.
- 활성 자동매매 설정과 runtime strategy cycle은 `intraday_momentum`, `trend_following`, `factor_investing` 기준입니다.
- `dual_momentum` 원장 데이터는 historical record로 유지하며 `intraday_momentum`으로 migration하지 않습니다.
- `intraday_momentum` 교체 상세 이력과 검증 기준은 `docs/plans/intraday_momentum_replacement_plan.md`를 기준으로 관리합니다.
...
- 이후 Phase 3 문서 동기화에서 PRD/DB_SCHEMA/phase3 plan 기준도 현재 운영 기능 중심 정의로 정리되었습니다.
