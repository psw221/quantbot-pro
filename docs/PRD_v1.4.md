...
- [x] Access Token 발급 및 만료 시각 저장
- [x] 매일 08:00 KST 토큰 갱신
- [x] 갱신 실패 시 3회 재시도 후 Emergency Stop
- [x] VTS/PROD 토큰 분리
- [x] 토큰 값 비영속화
...
- [x] 신호 수신 → 포지션 사이징 → 주문 생성 → 브로커 전송 파이프라인
- [x] 시장가 / 지정가 지원
- [x] 미체결 재주문 및 최대 재시도 횟수 관리
- [ ] 분할 매수/매도 지원
- [x] 중복 주문 방지 Lock
- [x] 부분체결 처리
- [x] 장 종료 전 미체결 주문 자동 취소

### 브로커 상태 동기화 요구사항

웹소켓 누락 또는 순서 역전 문제를 보완하기 위해 브로커 상태 동기화를 요구합니다.

- [x] **시장 운영 시간 중 10분 주기 브로커 폴링 수행**
- [x] 폴링 대상: 미체결 주문, 보유 수량, 가용 현금, 부분체결 누락 여부
- [x] 웹소켓 이벤트와 폴링 결과가 다르면 정합성 mismatch로 기록
- [x] mismatch 발생 시 신규 주문을 일시 중단하고 재동기화 수행
- [x] 브로커 스냅샷을 저장하여 DR 및 추적에 활용
...
- 국내 시장 제약 검증은 `execution.market_constraints.MarketConstraintValidator`가 담당합니다.
- 자동매매 cycle은 risk/sizing 이후 order candidate 생성 직전에 국내 제약을 검증합니다.
- 가격 제한폭은 전일 종가 대비 ±30%를 기본값으로 사용합니다.
- 동시호가 기본 차단 구간은 `08:30~09:00 KST`, `15:20~15:30 KST`입니다.
- 공매도 방지는 내부 전략 포지션 수량을 초과하는 매도 주문을 차단합니다.
- 단기 과열 및 거래정지성 이벤트는 `event_calendar`에서 `kr_overheated`, `kr_trading_halt` 이벤트 또는 대응 action으로 읽어 신규 매수를 차단합니다.
- T+2 결제 자금 반영은 브로커 주문가능금액 또는 최신 portfolio snapshot의 `cash_krw`를 기준으로 보수적으로 검증합니다.
